...
def calcfuncontentsexp(sm_expo, contentsvuln, taxonomymap,countrycontentsvuln):
    ...
    for i in taxonomymap['conversion'][tax_idx]:
        curr_idx = contentsvuln.loc[contentsvuln['Building_class'] == i].index
        vuln_idx.append(curr_idx[0])
    df1 = pd.DataFrame(contentsvuln.iloc[vuln_idx]).reset_index()
    del df1['index']
    ...
    if(len(sk)!=0):
        ...
        sm_expo["COST_CONTENTS_USD"][sk]=0.7*sm_expo["TOTAL_REPL_COST_USD"][sk]
    ...
    result=cost_ratio*sm_expo["COST_CONTENTS_USD"]
    return result
