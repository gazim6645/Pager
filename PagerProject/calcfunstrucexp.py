...
def calcfunstrucexp(sm_expo, structvuln, taxonomymap,countrystructvuln):
    ...
    for i in taxonomymap['conversion'][tax_idx]:
        curr_idx = structvuln.loc[structvuln['Building_class'] == i].index
        vuln_idx.append(curr_idx[0])
    df1 = pd.DataFrame(structvuln.iloc[vuln_idx]).reset_index()
    del df1['index']
    ...
    result=cost_ratio*sm_expo["COST_STRUCTURAL_USD"]
    return result
