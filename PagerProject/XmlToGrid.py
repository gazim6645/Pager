...
class ShakeMapGrid(object):
    ...
    def point_shaking(self,
        ...
        if not self.in_grid(lon_min=lon, lon_max=lon, lat_min=lat, lat_max=lat):
        ...
        shaking = [point for point in self.grid[start:end] if
            ...
            #for point in shaking:
                #point['gps_distance'] = get_gps_distance(point['LAT'], point['LON'], self.lat, self.lon)

            #shakecast.app.grid.Point.sort_by = 'gps_distance'
...
def create_grid(shakemap=None, grid_file='grid.xml'):
    """
    ...
    """
    grid = ShakeMapGrid()
    grid_location = os.path.join(shakemap.directory_name, grid_file)
